...
class SigleLoss(nn.Module):
    ...
    def __init__(self, hyp, task=None):
        ...
        alpha1, gamma1, alpha2, gamma2, alpha3, gamma3 = (
            hyp["alpha1"],
            hyp["gamma1"],
            hyp["alpha2"],
            hyp["gamma2"],
...
class TotalLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        tversky_da_loss, tversky_ll_loss = self.seg_tver_da(
            out_da, seg_da
        ), self.seg_tver_ll(out_ll, seg_ll)
        focal_da_loss, focal_ll_loss = self.seg_focal(out_da, seg_da), self.seg_focal(
            out_ll, seg_ll
        )

        tversky_loss, focal_loss = (
            tversky_da_loss + tversky_ll_loss,
            focal_da_loss + focal_ll_loss,
        )
        loss = focal_loss + tversky_loss
        return focal_loss.item(), tversky_loss.item(), loss
