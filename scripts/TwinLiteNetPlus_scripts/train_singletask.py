...
from utils import netParams, poly_lr_scheduler, save_checkpoint, train, val_one
...
def train_net(args, hyp):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        (
            print(f"Driving Area Segment: mIOU({segment_results[2]:.3f})")
            if args.task == "DA"
            else print(
                f"Lane Line Segment: Acc({segment_results[0]:.3f}) IOU({segment_results[1]:.3f})"
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["DA", "LL"],
        default=None,
        help="DA for drivable area, LL for lane line",
    )
    parser.add_argument(
        "--hyp",
        type=str,
